...
Use fresh input to audit the remaining high-fan-out contracts: Alt-rotate behavior, text inspector field focus during pointer edits, and geometry parity in standalone/HyperFrames/Remotion. Accept a code change only when a reproducible break is found and its cross-surface proof is added.
