...
function prepareNanaStudioHtml(source, animillTheme) {
  ...
  if (layoutStart >= 0 && mainStart > layoutStart && generatorStart > mainStart && generatorEnd > generatorStart && scriptTailStart > generatorEnd) {
    ...
    const animillChrome = '<div class="animill-nana-cursor" aria-hidden="true"></div>';
  ...
  html = html.replace('</head>', '<style id="animill-source-theme">' + animillTheme + '</style><link rel="stylesheet" href="/nana-animill.css?rev=borderless-1"></head>');
  // Remove the old button's inline dimensions, and give both actions native semantics.
  html = html.replace(/(<button id="gen-btn" onclick="runGenerator\(\)") style="[^"]*"/, '$1 class="animill-action primary"');
  html = html.replace(/<div class="btn btn-primary" id="veo-auth-btn" onclick="connectGoogleVeo\(\)">([^<]*)<\/div>/,
    '<button type="button" class="animill-action" id="veo-auth-btn" onclick="connectGoogleVeo()">$1</button>');
  html = html.replace(/body\.theme-[\w-]+\s*\{[^}]*\}/g, '');
...
app.get('/nana-studio-master.html', async (_request, response) => {
  ...
  const [source, reference] = await Promise.all([
    readFile(nanaStudioSource, 'utf8'),
    readFile(path.join(root, 'public', 'animill.html'), 'utf8')
  ]);
  // ANIMILL owns the palette, ambient background and control geometry.
  const rules = [':root', 'body:before', 'body:after',
    'button,.tabBtn,.buttonLike', '.primary', '.cursor'];
  const theme = rules.map(selector => {
    const start = reference.indexOf(selector + '{');
    if (start < 0) throw new Error('Missing ANIMILL UI rule: ' + selector);
    const rule = reference.slice(start, reference.indexOf('}', start) + 1);
    return selector === '.cursor' ? rule.replace('.cursor', '.animill-nana-cursor') : rule;
  }).join('\n');
  response.type('html').send(prepareNanaStudioHtml(source, theme));
});
