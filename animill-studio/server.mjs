...
(function () {
  ...
  if (cursor && halo && window.matchMedia('(pointer:fine)').matches) {
    ...
    document.addEventListener('pointerover', function (event) {
      if (event.target.closest('button, .btn, .pill, .gen-mode-btn, .engine-tile, summary, input, textarea, select')) halo.classList.add('is-hover');
    }, { passive: true });
    document.addEventListener('pointerout', function (event) {
      if (event.target.closest('button, .btn, .pill, .gen-mode-btn, .engine-tile, summary, input, textarea, select')) halo.classList.remove('is-hover');
    }, { passive: true });
    paintCursor();
  }
