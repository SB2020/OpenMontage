...
(function () {
  ...
  var halo = document.querySelector('.animill-nana-halo');
  ...
  if (cursor && halo && window.matchMedia('(pointer:fine)').matches) {
    ...
    document.addEventListener('pointerover', function (event) {
      var target = event.target.closest && event.target.closest('button, .btn, .pill, .gen-mode-btn, .engine-tile, summary, input, textarea, select');
      if (target) halo.classList.add('is-hover');
    }, { passive: true });
    document.addEventListener('pointerout', function (event) {
      var target = event.target.closest && event.target.closest('button, .btn, .pill, .gen-mode-btn, .engine-tile, summary, input, textarea, select');
      var next = event.relatedTarget && event.relatedTarget.closest && event.relatedTarget.closest('button, .btn, .pill, .gen-mode-btn, .engine-tile, summary, input, textarea, select');
      if (target && !next) halo.classList.remove('is-hover');
    }, { passive: true });
    paintCursor();
  }
